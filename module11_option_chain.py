...
def _parse_expiry(raw: list[dict], expiry_date: date) -> dict:
    ...
    fmt_alpha   = expiry_date.strftime("%d-%b-%Y")  # 28-Apr-2026
    ...
    for rec in raw:
        ...
        strikes[k] = {
            ...
            "call_price": float(ce.get("lastPrice", 0)) or float(ce.get("sellPrice1", 0)) or float(ce.get("buyPrice1", 0)) or 0.0,
            "put_price":  float(pe.get("lastPrice", 0)) or float(pe.get("sellPrice1", 0)) or float(pe.get("buyPrice1", 0)) or 0.0,
