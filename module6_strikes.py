...
def predict_range(feature_row: pd.Series) -> dict:
    ...
    if regime not in cfg.regime_alphas:
        raise KeyError(
            ...
            f"Available: {list(cfg.regime_alphas.keys())}. "
            f"Check .env / config.py regime alpha configuration."
        )
    alpha_p10, alpha_p90 = cfg.regime_alphas[regime]
    ...
    if mapie_model is not None:
        ...
        log_range_sigma = (log_range_p90 - log_range_p10) / divisor
