...
def _load_config():
    """Load strike config from .env, including VIX baseline and put skew."""
    if _CONFIG_CACHE:
        return _CONFIG_CACHE["buffer"], _CONFIG_CACHE["wing"], _CONFIG_CACHE["vix_baseline"], _CONFIG_CACHE["put_skew"]

    env_path = BASE_DIR / ".env"
    load_dotenv(dotenv_path=env_path)
    buffer_pts = int(os.getenv("STRIKE_BUFFER_POINTS", 50))
    wing_pts = int(os.getenv("WING_WIDTH_POINTS", 200))
    ...
    if vix_baseline is None:
        ...
        if vix_path.exists():
            try:
                ...
                # Use last 252 trading days for 52-week mean
                vix_baseline = float(vix_df["close"].tail(252).mean())
    ...
    _CONFIG_CACHE["wing"] = wing_pts
    _CONFIG_CACHE["vix_baseline"] = vix_baseline
    _CONFIG_CACHE["put_skew"] = put_skew_pts
    return buffer_pts, wing_pts, vix_baseline, put_skew_pts
...
def generate_strikes(
    ...
    """
    buffer_pts, wing_width, vix_baseline, loaded_put_skew = _load_config()
    ...
    effective_buffer = np.clip(buffer_pts * vix_scalar, 30, 150)
    ...
    upper_price = current_close + half_range_p90 + effective_buffer
    ...
    if put_skew_pts > 0:
        ...
        logger.debug("No put skew applied (put_skew_pts == 0)")
