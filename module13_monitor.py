...
FEATURE_DRIFT_CATEGORY_MIN = 3
FEATURE_CATEGORIES: dict[str, tuple[str, ...]] = {
    "vol":   ("atr_", "parkinson_vol", "garman_klass_vol", "realized_vol_5min",
              "realized_vol", "garch_", "vol_", "iv_"),
    "price": ("range_", "bb_width", "bb_", "vix_zscore", "vix_level", "vix_change",
              "close", "spot", "return_", "gap_"),
    "macro": ("usd_inr", "us_10y", "us_", "crude", "spx_", "vix_futures",
              "macro_", "ust_", "tnx"),
    "calendar": ("dow_", "month_", "week_", "is_event", "days_to_"),
}
...
def run_monitor() -> dict:
    ...
    if results is not None and len(results) > 0:
        ...
        if "actual_log_range" in results.columns and "log_range_p10" in results.columns:
            ...
            if len(rolling_cov) > 0 and pd.notna(rolling_cov.iloc[-1]):
    ...
    if feat_df is not None and len(feat_df) > DRIFT_WINDOW_WEEKS * 2:
        ...
        drift_by_category: dict[str, list[str]] = {k: [] for k in FEATURE_CATEGORIES}
        drift_by_category["other"] = []
        ...
        for col in numeric_cols:
            ...
            if drift_info["drift_detected"]:
                ...
                cat = _categorize_feature(col)
                drift_by_category.setdefault(cat, []).append(col)
        ...
        report["feature_drift"]["drift_by_category"] = {
            cat: feats for cat, feats in drift_by_category.items() if feats
        }

        for cat, feats in drift_by_category.items():
            if len(feats) >= FEATURE_DRIFT_CATEGORY_MIN:
                report["alerts"].append(
                    f"FEATURE_DRIFT: category={cat} count={len(feats)} "
                    f"features={','.join(feats[:5])}{'...' if len(feats) > 5 else ''}"
                )
    ...
    elif any(a.startswith("FEATURE_DRIFT: category=") for a in report["alerts"]):
