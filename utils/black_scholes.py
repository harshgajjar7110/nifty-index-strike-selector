...
def black_scholes_delta(
    ...
    if option_type == "call":
        return np.exp(-q * T_years) * norm.cdf(d1)
    else:
        return np.exp(-q * T_years) * (norm.cdf(d1) - 1)
...
def implied_volatility(
    ...
    def objective(sigma):
        return black_scholes_price(S, K, T_years, sigma, option_type, r, q) - price

    try:
        # Volatility typically bounds between 0.1% and 500%
