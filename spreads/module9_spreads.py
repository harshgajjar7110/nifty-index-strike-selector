...
def _get_strike_iv(
    ...
    if mkt_price > 0.5:
        ...
        if expiry_str:
            ...
            T = 11 / 365.0
        computed_iv = implied_volatility(mkt_price, spot, k, T, side, 0.065, 0.015)
    ...
    if price_lower and price_upper:
        ...
        if expiry_str:
            ...
            T = 11 / 365.0
        iv_lo = implied_volatility(p_lo, spot, k_lo, T, side, 0.065, 0.015)
        iv_hi = implied_volatility(p_hi, spot, k_hi, T, side, 0.065, 0.015)
...
def generate_all_spreads(
    ...
    min_ev_proxy = cfg.min_ev_proxy_pts
    if min_ev_proxy != 0:
        before = len(all_spreads)
        all_spreads = [s for s in all_spreads if s["ev_proxy"] >= min_ev_proxy]
        dropped_ev = before - len(all_spreads)
        if dropped_ev > 0:
            logger.info(f"EV filter: dropped {dropped_ev} spreads (ev_proxy < {min_ev_proxy} pts)")
